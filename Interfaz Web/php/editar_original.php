...
if (!empty($_POST)) {
    ...
    // Verificar que el curso perteneciente esté en la lista de cursos disponibles
    if (!in_array($cursoPerteneciente, array_column($cursos_disponibles, 'ID_Curso'))) {
        $msg = 'El curso seleccionado no es válido.';
    } else {
        // Verificar que la tarjeta NFC seleccionada esté en la lista de tarjetas disponibles
        if ($nfcTag !== null && !in_array($nfcTag, array_column($tarjetas_disponibles, 'ID_Tarjeta_NFC'))) {
            $msg = 'La tarjeta NFC seleccionada no es válida.';
        } else {
